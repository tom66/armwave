...
while True:
	...
	t0 = time.time()
	buf = armwave.test_get_buffer()
	t1 = time.time()
	print("test_get_buffer:         %5.1f ms" % ((t1 - t0) * 1000))
	...
	t0 = time.time()
	pbuf = GdkPixbuf.Pixbuf.new_from_bytes(gbytes, GdkPixbuf.Colorspace.RGB, True, 8, 2048, 256, 2048 * 4)
	t1 = time.time()
